...
if not os.path.exists('wordPatterns.py'):
    makeWordPatterns.main()#Create the wordPattern.py file
import wordPatterns
...
def gerBlankCipherletterMapping():
...
def intersectMappings(mapA, mapB):
    ...
    for letter in LETTERS:
        ...
        #just copy the other map entireley
        if mapA[letter] == []:
...
def removeSolvedLetersFromMapping(letterMapping):
    ...
    while loopAgain:
        ...
        for cipherletter in LETTERS:
            for s in solvedLetters:
                if len(letterMapping[cipherletter]) != 1 and s in letterMappping[cipherletter]:
...
def hackSimpleSub(message):
    ...
    #Remove any solved letters from other lists
...
def decryptWithCipherletterMapping(ciphertext, lettermapping):
    ...
    for cipherletter in LETTERS:
        if len(letterMapping[cipherletter]) == 1:
            ...
            ciphertext= cipher.replace(cipherletter.upper(), '_')
    key = ''.join(key)
