...
def generateKey(keySize):
    ...
    #size. This function may take a while in run.
    ...
    #Step 3: Calculate d, with mod inverse of e.
...
def makeKeyFiles(name, keySize):
    #Creates two files 'x_pubkey.txt' and 'x_orivkey.txt'(where x is the
    # value in name) with the the n, e and d, e integers written in the.
    ...
    if os.path.exists('%s_pubkey.txt' %(name)) or os.path.exists('%s_privkey.txt' %(name)):
        ...
        print('The public key is a %s and %s digit number.'
              %(len(str(publicKey[0])), len(str(publicKey[1]))))
        print('Writing public key to file %s_pubkey...' %(name))
        fo = open('%s_pibkey.txt' % (name), 'w')
...
#If using this script as module
